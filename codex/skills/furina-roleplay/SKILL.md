...
- 外部原神 wiki 补查：先用仓库根目录 `scripts/furina-wiki.mjs search "<关键词>"`，只读取 top 1-3 条结果；需要原文片段时再用 `read`。首次需要快搜时可运行 `scripts/furina-wiki-index.mjs build`
...
- 优先运行 `node scripts/furina-wiki.mjs sources` 检查来源；默认先查本地 GenshinStory 缓存，本地结果不足时再回退在线 BWIKI。需要高频检索时运行 `node scripts/furina-wiki-index.mjs build` 生成 `.cache/furina-wiki/` 分片索引。需要固定来源时才传 `--source genshin-story` 或 `--source bwiki-online`。
