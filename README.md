...
> 一个面向 Claude Code 与提示词运行时的芙宁娜·德·枫丹角色扮演项目，包含人格设定、OOC 行为规则、结构化角色知识库、记忆格式、记忆反思提示词，以及 Claude Code 原生斜杠命令。
...
本仓库用于构建一个更稳定、更沉浸的芙宁娜角色扮演体验。它不是一个传统可执行应用，而是一组可被 AI 客户端、RAG 流程或 Claude Code 斜杠命令调用的提示词与知识文件。
...
- Claude Code 原生命令：提供 `/furina`、`/furina-save`、`/furina-reflect` 三个命令文件。
