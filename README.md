...
本说明针对 `main` 分支。`main` 保持轻量，不再提交仓库内置 `vendor/GenshinStory` 快照；外部原神资料默认查询在线 BWIKI，本地 GenshinStory 只作为可选缓存。

| 项目 | `main` 分支 |
|------|-------------|
| 外部原神资料 | 默认查询在线 BWIKI；显式指定时可读取本地 GenshinStory 缓存 |
| 本地资料体积 | 轻量，不包含完整 GenshinStory 快照 |
| 查询性能 | 在线查询默认可用；本机有 GenshinStory 缓存时可生成 `.cache/furina-wiki/` 分片索引 |
| 深度探索 | 支持 `scripts/furina-explore.mjs` 最多 5 路并行探索，默认 10 分钟任务窗口 |
| 适用场景 | 安装简单、默认在线补查、可选本地资料增强 |

如果需要离线或半离线检索，请把 GenshinStory 缓存放在同级目录 `../genshinstory-cache`，或用 `GENSHIN_STORY_ROOT` / `--root` 指向其他本地路径。
...
`scripts/furina-wiki.mjs` 用于按需查询外部原神资料。本分支默认查询在线原神 BWIKI；如果本机有 GenshinStory 缓存且已生成 Markdown，也可以把它作为本地缓存来源。仓库本身不再提交 `vendor/GenshinStory`。
...
本地缓存默认查找同级目录 `../genshinstory-cache`。如果要改用其他 GenshinStory 路径，请设置 `GENSHIN_STORY_ROOT` 或传入 `--root` 覆盖；如果要显式固定在线 BWIKI，可传入 `--source bwiki-online`。
