...
| 外部原神 wiki 查询 | 通过 `scripts/furina-wiki.mjs` 按需查询在线原神 BWIKI，也可使用本地 GenshinStory 缓存 |
...
记忆格式采用 `version: "2.0"`，包含亲密度、交互状态、灵魂状态、核心记忆、背景笔记和睡眠巩固状态。反思 JSON 中的 `soul_state` 使用字符串值（`low` / `calm` / `active` / `excited`），记忆条目 ID 会按已有最大 `Mxxx` 稳定递增，避免压缩或删除后因数组位置变化而改号。完整字段说明见 [src/memory/memory_format.md](src/memory/memory_format.md) 与 [src/memory/cognitive_memory.md](src/memory/cognitive_memory.md)。
...
- `config/settings.json` 记录建议运行参数、记忆上限、主动回忆阈值、睡眠巩固软目标/硬上限和 OOC 安全开关；`scripts/furina-memory.mjs` 会读取其中的关键记忆阈值，外部运行时也可以把它作为配置来源。
- `config/wiki_sources.json` 由 `scripts/furina-wiki.mjs` 读取，用于选择在线 BWIKI 或本地 GenshinStory 缓存。
