...
2. **边界强保护**：`type=boundary` 默认按 `priority=3` 处理；若输入误标为 1 或 2，输出时校正为 3。
3. **合并同主题**：同一 `type` 下内容相近的多条，合并为一条更抽象的陈述（≤25 字）。
4. **跨类型合并**：不同 `type` 但描述同一用户维度的，合并后选取最能概括的 `type`。
5. **删除低价值低优先级**：`priority=1` 且内容已被其他条目覆盖的直接删除。
6. **弱记忆衰减**：`strength<40` 且长期未被访问的条目可降低强度或删除。
7. **优先级上调**：若同一事实在多条中反复出现，合并后 `priority` 上调一级（最高 3）。
8. **ID 复用**：合并时保留被合并组中数字最小的 ID，不新建 ID。

`removed_ids` 记录被删除的旧 ID，`decayed_ids` 记录只降权不删除的旧 ID；不要在压缩输出中使用 `obsolete_ids`，该字段只属于反思流程。
