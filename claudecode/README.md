...
node scripts/furina-wiki.mjs search "芙宁娜 传说任务" --top 3 --build-index
...
- **外部 wiki 运行时 `scripts/furina-wiki.mjs`**：按 `config/wiki_sources.json` 默认先查本地 GenshinStory 缓存，本地不足时回退在线 BWIKI
- **本地 wiki 索引与探索 `scripts/furina-wiki-index.mjs` / `scripts/furina-explore.mjs`**：生成 `.cache/furina-wiki/` 分片索引，并为复杂问题提供最多 5 路并行证据探索
